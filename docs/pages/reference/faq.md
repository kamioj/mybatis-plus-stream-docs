...
**解决**：参考 [安装 - 分页插件配置](/pages/quickstart/install#分页插件配置)，添加 `MybatisPlusInterceptor` Bean。
...
```java
// like: 需要手动加 % 通配符
where -> where.like(User::getUsername, "%user%")

// likeDefault: 自动在前后加 %
where -> where.likeDefault(User::getUsername, "user")
...
```java
// 这两个是等价的
userService.list(where -> where.eq(User::getRole, "user"));
userService.stream().filter(where -> where.eq(User::getRole, "user")).collect(Collectors.toList());
```
...
userService.listJoin(
    ...
    where -> {},
    select -> select.select(User::getId, DTO::getUserId)
...
where -> where.likeDefault(keyword != null && !keyword.isBlank(), User::getUsername, keyword)
