...
int deleted = userService.executableStream()
    .filter(where -> where.eq(User::getUsername, "test_user"))
    .executeDelete();
```
...
int result = userService.executableStream()
    .duplicate(dup -> dup.duplicate(User::getCreditScore))
    .executeInsert(user);
...
service.executableStream()
    .filter(where -> where.withDeleted().eq(Entity::getName, "deleted_item"))
