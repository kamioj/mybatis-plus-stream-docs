...
| 参数类型 | 简写 | 作用 | 示例 |
|---------|------|------|------|
| `Consumer<NormalWhereLambdaQueryWrapper>` | `w -> ...` | WHERE 条件 | `w -> w.eq(User::getRole, "admin")` |
| `Consumer<OrderLambdaQueryWrapper>` | `o -> ...` | 排序 | `o -> o.orderDesc(User::getId)` |
| `Consumer<SelectLambdaQueryWrapper<R>>` | `s -> ...` | SELECT 映射 | `s -> s.select(User::getName, DTO::getName)` |
| `Consumer<JoinLambdaQueryWrapper<T>>` | `j -> ...` | JOIN 连表 | `j -> j.leftJoin(Order.class, ...)` |
| `Consumer<GroupLambdaQueryWrapper>` | `g -> ...` | GROUP BY | `g -> g.groupBy(User::getRole)` |
| `Consumer<NormalSetLambdaQueryWrapper>` | `s -> ...` | SET 赋值 | `s -> s.set(User::getScore, 100)` |
| `Function<GroupFunctionLambdaQueryWrapper, V>` | `x -> ...` | 函数表达式 | `x -> x.count()` |
